...
namespace Bhp.SmartContract
{
    public class StandardService : IDisposable, IInteropService
    {
        ...
        private StackItem DeserializeStackItem(BinaryReader reader, ApplicationEngine engine)
        {
        ...
        private bool Contract_Call(ApplicationEngine engine)
        {
            ...
            {
                stacks.Push(engine.CurrentContext.EvaluationStack.Pop());
            }
            ExecutionContext context_new = engine.LoadScript(contract.Script, engine.CurrentContext.ScriptHash, 1);
            for (int i = 0; i < stacks.Count;)
            {
                context_new.EvaluationStack.Push(stacks.Pop());
            }
            return true;
        }
